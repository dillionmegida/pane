...
const BreadcrumbPart = styled.span`
  cursor: pointer;
...
export function SizeVisualizerModal({ data, onClose }) {
  ...
  const [nodeStack, setNodeStack] = useState([]); // breadcrumb trail of nodes
  ...
  const enterDir = (child) => {
    setNodeStack(s => [...s, child]);
  };

  const navigateToIndex = (idx) => {
    setNodeStack(s => s.slice(0, idx + 1));
  };

  const handleFileClick = (child, parentNode) => {
    // Reveal the file in the pane, building columns from the basePath outward
  ...
  return (
    <Overlay onClick={onClose}>
      <ResizableModalBox width="640px" height="520px" onClick={e => e.stopPropagation()}>
        <ModalHeader>
        ...
        <ModalBody pad="14px">
          {loading && (
            <div style={{ textAlign: 'center', padding: 60, color: theme.accent?.blue || '#4A9EFF' }}>
          ...
          {!loading && currentNode && (
            <>
              ...
              <div style={{ fontSize: 11, color: theme.text?.tertiary, marginBottom: 10, display: 'flex', gap: 12 }}>
                ...
                <span>{sorted.length} items shown{currentNode.children?.length > 50 ? ` (of ${currentNode.children.length})` : ''}</span>
                {currentNode !== tree && (
                  <span
                    style={{ cursor: 'pointer', color: theme.accent?.blue || '#4A9EFF' }}
                    onClick={() => setNodeStack([tree])}
                  >
                    ↑ Back to root
                  </span>
                )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {sorted.map((child, i) => {
                  ...
                  return (
                    <ItemRow
                      key={child.path}
                      onClick={() => child.isDirectory ? enterDir(child) : handleFileClick(child, currentNode)}
                      title={child.path}
                    >
