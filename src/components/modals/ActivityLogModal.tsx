      ...
      <ResizableModalBox width="700px" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
        ...
        <ModalBody pad="12px">
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            <Input value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} placeholder="🔍 Search logs..." style={{ flex: 1 }} />
          ...
          {logs.map(log => {
            ...
            return (
              <div key={log.id} style={{ display: 'flex', gap: 10, padding: '7px 0', borderBottom: '1px solid #222', fontSize: 11 }}>
                <span style={{ flexShrink: 0, fontSize: 14 }}>{ACTION_ICONS[log.action] || '•'}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: '#e8e8ed', marginBottom: 2 }}>{log.description}</div>
                  {log.source && (
                    <div style={{ color: '#5a5a6b', fontFamily: 'monospace', fontSize: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
