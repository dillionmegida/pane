...
interface ResizableModalBoxProps {
...
export const ResizableModalBox = ({
  children,
  width: initialWidth = '700px',
  height: initialHeight = 'auto',
  minWidth = 300,
  minHeight = 200,
  ...props
}: ResizableModalBoxProps) => (
  <ModalBox
    width={initialWidth}
    height={initialHeight}
    style={{ resize: 'both', overflow: 'auto', minWidth: `${minWidth}px`, minHeight: `${minHeight}px` }}
...
export const ModalBody = styled.div<{ pad?: string }>`
  flex: 1;
  overflow-y: auto;
...
export const Input = styled.input.withConfig({
  shouldForwardProp: (prop) => !['mono', 'width'].includes(prop as string),
  ...
  padding: 5px 8px;
  outline: none;
  font-family: ${(p: { mono?: boolean }) => p.mono ? 'monospace' : 'inherit'};
  width: ${(p: { width?: string }) => p.width || 'auto'};
...
export const Row = styled.div<{ gap?: string; align?: string; mb?: string }>`
  display: flex;
  gap: ${p => p.gap || '8px'};
