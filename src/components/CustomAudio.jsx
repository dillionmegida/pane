...
const Bar = styled.div`
  ...
  ${p => p.playing
    ? css`animation: ${BOUNCES[p.idx % BOUNCES.length]} ${0.55 + (p.idx * 0.07)}s ease-in-out infinite;
...
const CustomAudio = forwardRef(function CustomAudio(
  ...
  useImperativeHandle(outerRef, () => ({
    ...
    set currentTime(v) { if (audioRef.current) audioRef.current.currentTime = v; },
  }));
