...
const makePane = (id: string, overrides: Record<string, any> = {}): any => {
  ...
  return {
    ...
    tabs: overrides.tabs !== undefined ? overrides.tabs : [tab],
  };
};
...
describe('Tabs - Switching Tabs', () => {
  beforeEach(() => {
    ...
    const tab2 = makeTab({
      ...
      label: 'Downloads',
      files: [{ name: 'archive.zip', isDirectory: false, size: 5000, modified: new Date().toISOString(), extension: 'zip', path: '/Users/john/Downloads/archive.zip' }],
      viewMode: 'column',
      sortBy: 'name',
  ...
  test('switching to same tab should be a no-op', () => {
    ...
    const paneBefore = result.current.panes.find((p: any) => p.id === 'left');
    const pathBefore = paneBefore.path;

    act(() => { result.current.switchTab('left', 0); });

    const paneAfter = result.current.panes.find((p: any) => p.id === 'left');
...
describe('Tabs - Closing Tabs', () => {
  ...
  test('closing active tab should switch to adjacent tab', () => {
    ...
    act(() => { result.current.closeTab('left', 1); });
  ...
  test('closing non-active tab after active should not change activeTab', () => {
    ...
    act(() => { result.current.closeTab('left', 2); });
  ...
  test('should not close last remaining tab', () => {
    ...
    let pane = result.current.panes.find((p: any) => p.id === 'left');
    expect(pane.tabs).toHaveLength(1);

    act(() => { result.current.closeTab('left', 0); });

    pane = result.current.panes.find((p: any) => p.id === 'left');
    expect(pane.tabs).toHaveLength(1);
  });
});
...
describe('Tabs - State Isolation', () => {
  ...
  test('column state should be independent per tab', () => {
    ...
    const pane2 = result.current.panes.find((p: any) => p.id === 'left');
...
describe('Tabs - Session Persistence', () => {
  ...
  test('saveSession should include full tab state', () => {
    ...
    expect((window as any).electronAPI.storeSet).toHaveBeenCalledWith('session', expect.objectContaining({
  ...
  test('saveSession should include tab columnState', () => {
    ...
    const savedCall = (window as any).electronAPI.storeSet.mock.calls.find((c: any) => c[0] === 'session');
    expect(savedCall).toBeTruthy();
    const session = savedCall[1];
    const leftPane = session.panes.find((p: any) => p.id === 'left');
...
describe('Tabs - Navigation Updates Active Tab', () => {
  beforeEach(() => {
    ...
    (window as any).electronAPI.readdir.mockResolvedValue({
      success: true,
      files: [{ name: 'file1.txt', isDirectory: false, size: 100, modified: new Date().toISOString(), extension: 'txt', path: '/Users/john/file1.txt' }],
    });
    (window as any).electronAPI.watcherStart.mockImplementation(() => {});
    (window as any).electronAPI.storeSet.mockResolvedValue(undefined);
...
describe('Tabs - Preview File Per Tab', () => {
  beforeEach(() => {
    ...
    const tab1 = makeTab({ id: 'tab-1', path: '/Users/john/Documents', basePath: '/Users/john/Documents', label: 'Documents', previewFile: file1 });
    const tab2 = makeTab({ id: 'tab-2', path: '/Users/john/Downloads', basePath: '/Users/john/Downloads', label: 'Downloads', previewFile: file2 });
  ...
  test('switching to tab with no preview should close preview pane', () => {
    ...
    act(() => {
      useStore.setState((s: any) => ({
        panes: s.panes.map((p: any) => ({
          ...p,
          tabs: p.tabs.map((t: any, i: number) => i === 1 ? { ...t, previewFile: null } : t),
        })),
      }));
    });
  ...
  test('setPreviewFile should save to active tab', () => {
    ...
    const newFile = { name: 'new.pdf', path: '/Users/john/Documents/new.pdf', isDirectory: false, size: 0, extension: 'pdf', modified: new Date().toISOString() };
