...
describe('Multi-Select - Shift Range Selection', () => {
  ...
  test('should select range from fileA to fileD with Shift', () => {
    ...
    act(() => {
      const files = result.current.panes.find((p: any) => p.id === 'left').files;
      const selectedFiles = result.current.panes.find((p: any) => p.id === 'left').selectedFiles;
      const fileIndex = files.findIndex((f: any) => f.path === '/test/fileD.txt');
      const selArray = [...selectedFiles];
      const lastIdx = files.findIndex((f: any) => f.path === selArray[selArray.length - 1]);
      const [start, end] = [Math.min(fileIndex, lastIdx), Math.max(fileIndex, lastIdx)];
      const range = files.slice(start, end + 1).map((f: any) => f.path);
  ...
  test('should select range in reverse order (fileD to fileA)', () => {
    ...
    act(() => {
      const files = result.current.panes.find((p: any) => p.id === 'left').files;
      const selectedFiles = result.current.panes.find((p: any) => p.id === 'left').selectedFiles;
      const fileIndex = files.findIndex((f: any) => f.path === '/test/fileA.txt');
      const selArray = [...selectedFiles];
      const lastIdx = files.findIndex((f: any) => f.path === selArray[selArray.length - 1]);
      const [start, end] = [Math.min(fileIndex, lastIdx), Math.max(fileIndex, lastIdx)];
      const range = files.slice(start, end + 1).map((f: any) => f.path);
...
describe('Multi-Select - Combined Cmd and Shift Selection', () => {
  ...
  test('should preserve existing selections when shift-selecting a range', () => {
    ...
    act(() => {
      const files = result.current.panes.find((p: any) => p.id === 'left').files;
      const selectedFiles = result.current.panes.find((p: any) => p.id === 'left').selectedFiles;
      const fileIndex = files.findIndex((f: any) => f.path === '/test/fileF.txt');
      const selArray = [...selectedFiles];
      const lastIdx = files.findIndex((f: any) => f.path === selArray[selArray.length - 1]);
      const [start, end] = [Math.min(fileIndex, lastIdx), Math.max(fileIndex, lastIdx)];
      const range = files.slice(start, end + 1).map((f: any) => f.path);
  ...
  test('should handle multiple shift-selections preserving all previous selections', () => {
    ...
    act(() => {
      const files = result.current.panes.find((p: any) => p.id === 'left').files;
      const selectedFiles = result.current.panes.find((p: any) => p.id === 'left').selectedFiles;
      const fileIndex = files.findIndex((f: any) => f.path === '/test/fileC.txt');
      const selArray = [...selectedFiles];
      const lastIdx = files.findIndex((f: any) => f.path === selArray[selArray.length - 1]);
      const [start, end] = [Math.min(fileIndex, lastIdx), Math.max(fileIndex, lastIdx)];
      const range = files.slice(start, end + 1).map((f: any) => f.path);
