...
const isPreviewableExt = (ext) =>
  PREVIEW_TYPES.imageExts.includes(ext) ||
  PREVIEW_TYPES.videoExts.includes(ext) ||
  PREVIEW_TYPES.audioExts.includes(ext) ||
  ext === 'pdf';

// ── PREVIEW_TYPES detection ───────────────────────────────────────────────────
...
describe('QuickPreview - file type detection', () => {
  test('image extensions are previewable', () => {
    ...
    images.forEach(ext => {
      expect(isPreviewableExt(ext)).toBe(true);
    });
  });
  ...
  test('non-previewable extensions return false', () => {
    ...
    others.forEach(ext => {
      expect(isPreviewableExt(ext)).toBe(false);
    });
  });
...
describe('QuickPreview - spacebar trigger logic', () => {
  ...
  test('does NOT open modal when inactive pane presses Space', () => {
    ...
    const handler = buildKeyHandler({
      paneId: 'left',
      activePane: 'right', // different pane is active
      selectedFiles: [img.path],
      files: [img],
      onOpen,
    });
...
describe('QuickPreview - Esc key close logic', () => {
  const buildEscHandler = ({ onClose }) => (e) => {
    if (e.key === 'Escape') {
      e.preventDefault();
      e.stopPropagation();
      onClose();
